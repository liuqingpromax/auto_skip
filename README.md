...
> git 标签必须与之保持一致（`v0.4.0` ↔ `versionName = "0.4.0"`）。

## v0.4.0 更新内容（倒计时按钮：「跳过3」与「3跳过」）

> 用户反馈：存在 `跳过n` 与 `n跳过` 两种形态（n 为倒计时，通常 1~5 秒），
> 要求学习模式也能学会这类按钮。

| 方向 | 本次改动 |
|---|---|
| **学习模式以前学不会「数字在前」** | 旧版 `keywordOf` 只剥**尾部**数字（`\s*\d+\s*$`）：`跳过3` 能归一成 `跳过`，但 `3跳过` 会**整串**被当关键词。学出来的规则只匹配「倒计时恰好等于 3」那一瞬间的文本，倒计时一变就失效 —— 等于学了个死规则 |
| **新增文本归一化模块** | `engine/SkipTextNormalizer.kt`：语义词优先（跳过/关闭/skip/close/✕…），否则剥掉**两端**的数字与连接符（空格、全角空格、`s/S`、`秒`、`后`、`·`、`.、:：,，、（）[]【】|/\-—`），两种排列都归一到同一个关键词 |
| **学出来的正则自带容错** | `patternForCountdownKeyword()` 生成三段交替：`跳过` \| `数字*连接符*跳过` \| `跳过连接符*数字+`，于是**不限倒计时数字、不限排列**都能命中；关键词本身含数字时退化为精确匹配，避免过度放宽 |
| **内置规则同步** | 倒计时条件改为同时覆盖两种排列，并兼容全角数字（`０-９`）、`s/秒/后`、英文 `in/after`（如 `Skip in 3s`）；`BuiltinRules.VERSION` 1 → **2**，保证老用户的内置规则会被升级 |
| **词表去重** | 关键词表从 `LearningController` 迁到 `SkipTextNormalizer.KEYWORDS`，`LearningController.SKIP_HINTS` 改为引用它，避免两处定义漂移 |
| **修掉一个会崩溃的 bug** | 连接符字符类里 `】-—` 被解析成**字符范围**，而 `】`(U+3011) → `—`(U+2014) 是逆序范围，Java/Python 正则直接抛异常。**这条分支正是「3跳过」才会走到的** —— 由新增的验证脚本抓出，连字符已移到字符类末尾 |
| **新增两道验证** | `tools/verify_countdown_patterns.py`（Python 等价模型，含从 Kotlin 源码读取内置规则，避免漂移）；`tools/java/VerifyCountdown.java`（用**真实 java.util.regex 引擎**验证，与 Kotlin `Regex` 同源） |
| **UI 文案** | 候选规则说明会明确告诉用户「不限倒计时数字」，并对倒计时按钮给出更高推荐度（94） |

## v0.3.0 更新内容（通用性：不再假设按钮在右上角）

> 用户反馈：**「跳过」不一定在右上角，叉号 ✕ 也可能是关闭按钮。** 本次针对这两点重做匹配逻辑。

| 方向 | 本次改动 |
|---|---|
| **位置从硬过滤改为加权** | 旧版把 `area: "top_right"` 写进内置规则，等于「按钮不在右上角就永远点不到」。现在 `area` 留空 = **全屏匹配**，位置只作加分：四角 25 分、顶部边缘 22、底部边缘 18、左右边缘 10、**屏幕中间 0 分**。四角一视同仁（不再只偏向右上），左下/右下/左上都能点；中间不加分则守住了防误触底线 |
| **认识叉号 ✕ 按钮** | 新增 `icon_button` 条件类型：匹配「**可点击 + 无文字 + 无描述 + 小尺寸（宽 ≤22% 屏宽、高 ≤18% 屏高）+ 位于边缘**」的图标节点。这类按钮以纯 ✕ 图标呈现，没有「跳过」二字，旧版完全无能为力。25 分基础分 + 位置分 + 尺寸分凑够阈值 |
| **关键词大幅扩展** | 中文「跳过/跳過/略过/关闭/关闭广告/跳过广告」、英文「skip/close/dismiss/cancel」、**叉号系字符「✕ ✖ ✗ × ⨯ ╳ ❌ ❎」**；倒计时正则改为 `\d+\s*秒?\s*(后)?\s*(跳过|关闭)` |
| **学习模式分角落识别** | 捕获兜底从「只找右上角」改为**四角 + 上下边缘全扫**；误点过滤从「只在上方 35%」放宽到**任意边缘区**（底部关闭按钮不再被判为误点）；位置兜底去掉「纵向超过 45% 直接放弃」的硬编码，改为按**实际点击角落**生成规则；无文字线索时新增「按纯图标按钮识别」候选 |
| **泛化兜底坐标** | `0.92, 0.08` → `0.92, 0.06`（右上角安全区内），且仅在节点点击全部失败时使用 |
| **内置规则可升级（重要修复）** | `RuleRepository.load()` 原先只在内置规则**缺失**时补齐。这意味着老用户升级后**永远拿不到改好的内置规则**——v0.2.x 改了匹配逻辑，用户本机仍是写死 `top_right` 的旧规则。现在内置规则带 `version`，版本偏低时自动升级为新内容（**保留用户的启用/禁用选择**），用户自建规则不受影响 |
| **区域选项细化** | 规则编辑器区域选择：不限（推荐）/ 右上 / 左上 / 右下 / 左下 / 顶部 / 底部；模板新增「× 关闭按钮」「纯图标关闭」「顶部通栏广告」「底部横幅广告」，并把默认区域从写死的右上改为**不限** |
| **可复现的自检脚本** | 新增 `tools/verify_matcher_model.py`：用等价模型验证位置分/尺寸分/icon_button 判定，覆盖四角、边缘、中间与防误触底线 |
