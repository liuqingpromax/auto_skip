...
> 当前版本 **v0.2.0**。本文档随版本更新，v0.2.0 的验收项见第 2 章。

## 1. 验收清单（对照说明书第十四章）

| # | 验收标准 | 实现位置 | 验证方法 |
|---|---|---|---|
| 1 | 安装后能引导开启无障碍服务 | `HomeScreen` 状态卡 + 「去开启无障碍服务」（三级兜底跳转）、分品牌路径提示 | 首启首页显示未开启 + 引导按钮 |
...
| B7 | 跳转兜底 | 在无标准无障碍入口的 ROM 上点「去开启」 | 依次尝试无障碍详情页 → 无障碍列表 → 系统设置首页，不崩不静默 |
