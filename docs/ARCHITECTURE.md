...
## 0.2 v0.4.0 变更摘要（倒计时按钮的两种排列）

> 触发原因（用户反馈）：存在 `跳过n` 与 `n跳过` 两种形态，n 为倒计时（通常 1~5 秒），
> 学习模式需要也能学会。

| 问题 | 原因（v0.3.0） | v0.4.0 做法 |
|---|---|---|
| 「数字在前」学不会 | `LearningController.keywordOf` 只剥**尾部**数字 `\s*\d+\s*$`：`跳过3` → `跳过`（可），但 `3跳过` → `3跳过`（整串当关键词）。学出的规则只匹配倒计时恰好等于 3 的那一瞬间，倒计时一变即失效 | 新增 `SkipTextNormalizer`：语义词优先，否则剥掉**两端**的数字与连接符；两种排列归一到同一关键词 |
| 学出的正则不够宽容 | 学习模式用 `Regex.escape(keyword)` 生成**精确字面量**，所以即便关键词对了，也只能匹配「跳过」二字、匹配不到「跳过5」 | `patternForCountdownKeyword()` 生成三段交替：`跳过` \| `数字*连接符*跳过` \| `跳过连接符*数字+`；关键词含数字时退化为精确匹配 |
| 内置规则同样只认一种排列 | 倒计时条件为 `\d+\s*秒?\s*(后)?\s*(跳过\|关闭)`，只覆盖「数字在前」且不含全角数字、英文 `in` | 条件改为两段交替覆盖两种排列，并兼容全角数字 `０-９`、`s/秒/后`、`in/after`；`BuiltinRules.VERSION` 1 → 2 |
| 字符类隐藏崩溃 | — | 连接符字符类写成 `【】-—`，被解析为 `】`(U+3011)→`—`(U+2014) 的**逆序范围**，Java/Python 正则编译即抛异常。**该分支只在「数字在前」时走到**，由新增验证脚本抓出；连字符已移到字符类末尾 |
| 词表两处定义 | `SKIP_HINTS` 在 `LearningController`，与归一化逻辑各写一份 | 统一到 `SkipTextNormalizer.KEYWORDS`，`SKIP_HINTS` 改为引用，避免漂移 |

## 0.3 v0.3.0 变更摘要（匹配通用性）
...
| type | 语义 | 关键字段 | 备注 |
|---|---|---|---|
| `text_regex` | 节点 `text` 正则匹配 | `pattern`、`score` | 关键词按字面量转义后写入；**倒计时关键词用 `SkipTextNormalizer.patternForCountdownKeyword()` 生成容错正则** |
| `desc_regex` | 节点 `contentDescription` 正则匹配 | `pattern`、`score` | 纯图标按钮常只有描述；同样支持倒计时容错正则 |
...
`engine/SkipTextNormalizer` 负责把按钮文本变成**稳定关键词**：

```
"跳过3" / "3跳过" / "5秒后跳过" / "跳过 5 秒" / "3s跳过"  →  "跳过"
"关闭2" / "2关闭"                                        →  "关闭"
"skip 3" / "3 skip" / "Skip in 3s"                       →  "skip"
"✕"                                                      →  "✕"
"3"（纯数字无语义）                                        →  null（不当关键词，避免误点）
```

`patternForCountdownKeyword("跳过")` 生成：

```
\Q跳过\E | [0-9０-９]*[连接符]*\Q跳过\E | \Q跳过\E[连接符]*[0-9０-９]+
```

三段交替分别对应「纯关键词」「数字在前」「数字在后」，
因此**学到某一瞬间的倒计时文本后，倒计时变化仍能命中**。
关键词自身含数字时退化为精确字面量，避免过度放宽。

⚠️ **连接符字符类的坑**：连字符必须放字符类**末尾**。
写成 `【】-—` 会被解析为 `】`(U+3011) → `—`(U+2014) 的逆序范围，正则编译直接抛异常；
而这条分支只在「数字在前」的文本上才会走到 —— 由 `tools/java/VerifyCountdown.java` 抓出。
...
## 6. 内置规则 JSON（v0.4.0：全屏匹配 + 叉号 + 倒计时双排列）
...
**与旧版的关键差异**：
1. 全部条件**不再带 `area`**（v0.3.0 起，位置改加权）；
2. 第 2、4 条倒计时条件**同时覆盖两种排列**（`3跳过` 与 `跳过3`），并兼容全角数字与 `in/after`（v0.4.0 起）。

> 本 JSON 由 `tools/verify_countdown_patterns.py` **直接读取**并验证，
> 所以改了规则后跑一下脚本就能确认没写错正则。
...
| 11 | **v0.3.0 通用性更新**：位置硬过滤改加权（四角/上下边缘/左右边缘）、新增 `icon_button` 纯图标/叉号 ✕ 识别、关键词扩充叉号系与英文、学习模式分角落识别、内置规则版本化升级、规则模板与区域选项细化、新增评分模型自检脚本 | ✔ 完成 |
| 12 | **v0.4.0 倒计时更新**：`SkipTextNormalizer` 归一化「跳过3 / 3跳过」两种排列、学习模式生成容错正则、内置规则覆盖双排列与全角数字/英文 in/after、词表统一、修掉字符类逆序范围崩溃、新增 Python 与真实 JVM 引擎两道验证 | ✔ 本次 |
